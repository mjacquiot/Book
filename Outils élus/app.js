...
window.handleLogin = async () => {
  ...
  if (passV === 'MAJA00') {
    ...
    alert("Mot de passe incorrect (le mot de passe de démonstration est MAJA00).");
  }
};
...
window.handleOCRUpload = async (e, sid) => {
  ...
  // Check if Tesseract is loaded, if not, load it dynamically
  if (typeof Tesseract === 'undefined') {
    const loader = document.getElementById('ocr-loader');
    const textInfo = document.getElementById('ocr-progress');
    const wasVisible = loader ? loader.style.display === 'block' : false;
    if (loader) loader.style.display = 'block';
    if (textInfo) textInfo.innerText = "Téléchargement du moteur d'OCR...";
    try {
      ...
      if (loader && !wasVisible) loader.style.display = 'none';
      alert(err.message);
      return;
    }
  }
